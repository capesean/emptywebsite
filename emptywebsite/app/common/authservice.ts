...
(function () {
    ...
    authService.$inject = ["$http", "$q", "$rootScope", "appSettings", "$filter"];
    function authService($http, $q, $rootScope, appSettings, $filter) {
        ...
        function isInRole(role) {
            if (!$rootScope.profile || !$rootScope.profile.roles) return false;

            //// passed as enum value, need to convert to name
            if (typeof (role) === "number") role = Models.Roles[role];

            // get the role id from the role name (from appSettings)
            var roleId;
            angular.forEach(appSettings.roles, function (r) {
                if (r.name === role)
                    roleId = r.id;
            });
            if (!roleId) return false;

            // check if user (identity) has the role id
            var found = false;
            angular.forEach($rootScope.profile.roles, role => {
        ...
        function isInAnyRole(roles: Models.Roles[]) {
            if (!$rootScope.profile || !$rootScope.profile.roles) return false;
