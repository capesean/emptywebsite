    ...
    var registerWatchers = function (labelWatcherArray, interpolationFunction, viewScope, step) {
        angular.forEach(getExpression(interpolationFunction), function (expression) {
            var watcher = viewScope.$watch(expression, function () {
                var maxLength = 50;
                var label = interpolationFunction(viewScope);
                label = label.substr(0, maxLength)
                if (label.length === maxLength && label.lastIndexOf(" ") >= 0) label = label.substr(0, Math.min(label.length, label.lastIndexOf(" "))) + "...";
    ...
    function BreadcrumbDirective($interpolate, $breadcrumb, $rootScope) {
        var $$templates = {
            ...
            bootstrap3: '<ol class="breadcrumb">' +
                ...
                '</li>' +
                '</ol>',
            bootstrap4: '<div><ol class="breadcrumb">' +
                '<li ng-repeat="step in steps" ng-class="{active: $last}" class="breadcrumb-item" ng-switch="$last || !!step.abstract">' +
                '<a ng-switch-when="false" href="{{step.ncyBreadcrumbLink}}">{{step.ncyBreadcrumbLabel}}</a>' +
                '<span ng-switch-when="true">{{step.ncyBreadcrumbLabel}}</span>' +
                '</li>' +
                '</ol></div>'
